...
long groups_base_num[MAX_GROUP_NUM+2];						//记录每个group的碱基数
...
{
	bool flag = false;//ture说明包含其他group
	...
	for(vector<size_t>::size_type id = 1;id != final_group_ui.size();++id)
	{
		...
		if(temp_group)
		{
			if(temp_group->group_id == final_group_ui[id])//删掉该节点
			{
				...
				{
					node->group = temp_group->next;
					temp_group->next = NULL;
					delete temp_group;
					temp_group = node->group;
				}
				flag = true;
			}else
			{
	...
	//2. 插入final_group_st 的首节点,如果已经包含，则不必处理
	if(flag)
	{
		GList * temp_group = node->group;
		GList *last_group = NULL;
...
void *tranvers_hash(void *ar)
{
	...
	while(true)
	{
		...
		for(size_t id = 0;id != 10000000;++id)
		{
			//退出线程
			if(temp_idx >= BUCKET_SIZE){pthread_exit(NULL);}
			if(hash[temp_idx])
			{
...
//reads输出到文件,追加方式
void output_reads_to_remain(string &lib_name,string group_no,string &file_format,string file_no,
			  string &seq_id,string &seq_base,string &seq_plus,string &seq_qual)
{
	string file_name;
	file_name.reserve(output_path.size() + 50);
...
//初始化group信息
void initialize_group(string library_name,int reads_type,string file_format)
{
	cout << "----------------------------" <<endl;
	system("date");
	cout << "begin to in initialize group infor..." <<endl;
	vector<Kmer *> *uniq_kmer_address = new vector<Kmer *>;		//为兼容大的contigs初始化，使用堆内存
	uniq_kmer_address->reserve(10000);				//uniq_kmer_address 设定为最多的情况
	char temp_group[5];
	string group_no;
	if(reads_type == 2)						//PE reads
	{
		...
		while(initialized_group != group_num_no && !reads_file1.eof())
		{
			...
			{
				sprintf(temp_group,"%d",initialized_group);
			...
			{
				output_reads_to_remain(library_name,"remain",file_format,"1",reads1_id,reads1_seq,reads1_plus,reads1_qual);
				output_reads_to_remain(library_name,"remain",file_format,"2",reads2_id,reads2_seq,reads2_plus,reads2_qual);
			}
		}
			...
			if(uniq_kmer_num >= initialize_threshold * reads_kmer_num)//该reads的uniq kmer数大于设定的阈值
			{
				...
				for(vector<Kmer *>::size_type idx = 0;idx != uniq_kmer_address->size();++idx)
				{
					...
					if(grp_head == NULL)
					{
						...
						while(grp_head->next){grp_head = grp_head->next;}//因为group是从小到大，所以是有序的i
						if(grp_head->group_id == initialized_group)
						{
							continue;//可能出现相同的uniq kmer在reads出现两次,已经加入了
							delete grp_head;
						}
						grp_head->next = grp;
					}
				...
				//{
				//	GList *grp_head = uniq_kmer_address->at(idx)->group;
				//	while(grp_head){cout << grp_head->group_id << " ";grp_head= grp_head->next;}
				//	cout <<"end" <<endl;
			 	//}
			}else
			{
...
void copyfile(string &f1_name,string &f2_name)
{
	if(DEBUG)cout << "merge " << endl << f1_name << endl <<  f2_name << endl;
        ...
        ofstream file1(f1_name.c_str(),std::ios::app);
        file1 << file2.rdbuf();
        file2.close();
        file1.close();
...
void select_groups(vector<Kmer *> *uniq_kmer_address,int min_kmer_num)
{
	...
	for(size_t id = 0;id != group_num_id;++id)
	{
		if(group_kmer_num[id] >= min_kmer_num && group_infor[id])
		{
			final_group_st.push_back(id);
			final_group_ui.push_back(ui_id);
		}
...
{
	unsigned int gid = 0;
	for(size_t id = 0;id != group_num_id;++id,++gid)
	{
		if(!group_infor[id])
		{
			uninitial_id = gid;
	...
	if(group_num == 0)					//不属于任何一个group
	{
		if(last_operate)
		{
			...
			}else//不能初始化group，输出到remain
			{
				...
				groups_base_num[group_num_id] += reads_base_num;
...
void* classify_one_lib(void *ar)
{
	...
	while(true)
	{
		...
		{
			++finish_reads_num;
			if(finish_reads_num % 100000 == 0)cout << "finish " << finish_reads_num << " reads." << endl;
			if(!file_format.compare("fastq"))
			{
				getline(reads_file1,reads1_id);
				getline(reads_file1,reads1_seq);
			...
			if(uniq_kmer_num < min_kmer_num)
			{
				...
				groups_base_num[group_num_id +1] += reads_base_num;
				continue;
			}
...
void classify_reads(vector<vector<string> > &all_reads_infor,int iterate_times)
{
	...
	for(vector<string>::size_type index = 0;index != all_reads_infor.size();++index)
	{
		...
		}else if(reads_type == 1)					//SE reads
		{
			...
			if(!initialize)
			{
				initialize_group(library_name,reads_type,file_format);
				initialize = true;
			}
		...
		//输入当前group的base信息
		for(size_t gi = 0;gi != group_num_id+2;++gi)
		{
